...
def process_cv(
    ...
    """Async CV pipeline : parse Claude → score → store + notify creator.

    Args:
        candidate_id     : UUID str du candidat à traiter
        position_id      : UUID str du poste cible (optionnel)
        bulk_import_id   : UUID str si appelé depuis un bulk CSV import
        creator_user_id  : UUID str du user qui a uploadé le CV (pour notif perso).
                           None → broadcast tenant admins/recruiters.
    """
    logger.info("cv_processing_start", candidate_id=candidate_id, position_id=position_id, bulk_import_id=bulk_import_id, creator_user_id=creator_user_id)
    ...
    try:
        ...
        # AI Act audit trail — décision IA "profile scoring" (Art. 13 transparence)
        from app.services.audit import log_ai_action_sync
        log_ai_action_sync(
            session,
            tenant_id=candidate.tenant_id,
            action="profile_scoring",
            entity_type="candidate",
            entity_id=str(candidate.id),
            model="claude-sonnet-4-5",
            model_version="2026-04",
            confidence_score=(candidate.profile_score / 100.0) if candidate.profile_score else None,
            summary=f"Score qualité CV calculé : {candidate.profile_score}/100",
            extra={"explanation": quality_data.get("explanation")},
        )
        ...
        # Notification au créateur (ou broadcast tenant si pas de creator)
        try:
            from uuid import UUID as _UUIDmod
            from app.services.notification_service import create_notification
            score_int = round(candidate.cv_score or 0)
            ctx_position = f" · {position.title}" if position else ""
            create_notification(
                session=session,
                tenant_id=candidate.tenant_id,
                user_id=_UUIDmod(creator_user_id) if creator_user_id else None,
                type="candidate.cv_analyzed",
                title=f"CV analysé : {candidate.name}",
                message=f"Score {score_int}/100{ctx_position}",
                data={
            ...
            logger.warning("notif_cv_analyzed_failed", candidate_id=candidate_id, exc_info=True)
        ...
        try:
            ...
            if cand and cand.pipeline_status == "cv_processing":
                ...
                # Notification d'échec au créateur
                try:
                    from app.services.notification_service import create_notification
                    create_notification(
                        session=session,
                        tenant_id=cand.tenant_id,
                        user_id=_UUID(creator_user_id) if creator_user_id else None,
                        type="candidate.cv_failed",
                        title=f"Échec analyse CV : {cand.name}",
                    ...
                    logger.warning("notif_cv_failed_failed", candidate_id=candidate_id, exc_info=True)
        except Exception:
            try:
                session.rollback()
            except Exception:
